...
Let's slurp forward in editors:
|===
| Scenario | emacs paredit | calva | notes
...
| ❌ This is not allowed in rewrite-clj, this is not a valid `:uneval` node.
...
| ⚠️️Technically, rewrite-clj could support this scenario because there are enough nodes to satisfy `:uneval`
