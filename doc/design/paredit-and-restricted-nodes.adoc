...
I am, therefore, I am giving the paredit API some love and attention.

Most fixes were straightforward, but how paredit operations should act on "restricted nodes" is more nuanced, hence this design doc.
...
Examples of unrestriced nodes that have any children are `:set` `+#{1 2 3}+`, `:vector` `[:a :b :c]`, `:list` `(foo bar baz)`, and `:map` `{:a 1 :b 2}`.
